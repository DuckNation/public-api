...
async def add_home(
    ...
    for h in player.saved_homes:
        if h.name.lower() == home.name.lower():
            raise HTTPException(status_code=400, detail="You already have a home by that name.")
    ...
    if isinstance(home, str):
        ...
        for h in player.saved_homes:
            if h.name.lower() == home.lower():
                ...
                await player.save(instance)
                return player.saved_homes
        raise HTTPException(status_code=400, detail="You do not have a home by that name.")
    else:
